...
-- Retrieve entire data from tables
...
-- Retrieve the types of pizza in each typecategory 

select pizza_types.category, count(pizza_types.name) as types 
from pizza_types group by category;

-- Retrieve the total pricing of each size category

select pizzas.size, round(sum(pizzas.price),2) as total_price
...
round(SUM(order_details.quantity * pizzas.price) / ( select  ROUND(SUM(order_details.quantity * pizzas.price),3) AS Total_revenue
FROM 
    order_details
        JOIN
